...
// --------------------------------------------------
	...
	if (defined('SWITCHER_DOSSIERS_SQUELETTES')) {
		foreach(explode(',',SWITCHER_DOSSIERS_SQUELETTES) as $skel) {
			if ( $skel == $dossier_squelettes ) $OK = 'X';
	...
	// Squelettes supplémentaires : tous les répertoires contenus dans 	$repertoire_squelettes_alternatifs
	   ...
	   if ($dh = opendir($repertoire_squelettes_alternatifs)) {
	       while (($file = readdir($dh)) !== false) {
	       		if ( (is_dir($repertoire_squelettes_alternatifs."/".$file)) AND ($file[0]!=".") ) {
	       			$rep = $repertoire_squelettes_alternatifs."/".$file ;
							if ( $rep == $dossier_squelettes ) $OK = 'X';
	       			$squelettes_alternatifs[$file]=$rep;
	       		}
	       }
	   closedir($dh);
	   }
		...
		$squelettes_alternatifs = array(
		'defaut' => '',
		'Defaut' => $dossier_squelettes);
		$OK = 'X';
	}
	
  if ( $dossier_squelettes != '' && $OK == '' ) $squelettes_alternatifs['Defaut'] = $dossier_squelettes ;
